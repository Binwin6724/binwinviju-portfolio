...
import useEncryptionAnimation from '../hooks/useEncryptionAnimation';
import useNameTranslation from '../hooks/useNameTranslation';

const Hero = () => {
  const description = `A passionate Senior Data Engineer and Full Stack Developer with 3.4 years of experience, specializing in building exceptional data solutions and web applications. Currently focused on developing automated data pipelines and creating efficient web applications.`;
  
  const { displayText: encryptedText } = useEncryptionAnimation(description, 2000);
  const { displayText: translatedName } = useNameTranslation(2000);
